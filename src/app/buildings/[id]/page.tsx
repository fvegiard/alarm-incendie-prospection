...
// Required for static export with dynamic routes: pre-render every building page at build time
...
export default async function BuildingDetailPage({ params }: BuildingDetailProps) {
  ...
  const priority = building.priority as Priority;
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-8 overflow-hidden rounded-3xl shadow-sm ring-1 ring-zinc-200 dark:ring-zinc-800">
          <div className="aspect-[16/7] w-full bg-zinc-200 dark:bg-zinc-800">
            <img
              src={streetViewUrl}
              alt={`Vue Street View de ${building.name}`}
        ...
        <div className="mb-8 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-3">
            <InfoRow icon={Building2} label="Nom" value={building.name} />
            <InfoRow icon={MapPin} label="Adresse" value={address} />
            <InfoRow icon={MapPin} label="Ville" value={city} />
            <InfoRow icon={Layers} label="Étages" value={building.floors} />
            <InfoRow icon={Calendar} label="Année de construction" value={building.year_built} />
            <InfoRow icon={Ruler} label="Hauteur" value={building.height_m ? `${building.height_m} m` : "—"} />
            <InfoRow icon={Building2} label="Type d'usage" value={building.usage_type} />
        ...
        <div className="mb-8 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
            <InfoRow
              ...
              value={building.fire_system_type || "À vérifier"}
            />
            <InfoRow
              icon={Flame}
              label="Statut du système"
              value={building.fire_system_status || "Inconnu"}
            />
          </div>
        </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p className="text-lg font-medium text-zinc-900 dark:text-white">
              {building.owner_name || "Non spécifié"}
            </p>
          </div>
