...
function inferCity(zone?: string | null): string | null {
  ...
  if (lower.includes("vieux-montréal") || lower.includes("quartier international")) {
    return "Montréal";
  }
  return "Montréal";
}
...
function classifyImageUrl(imageUrl: string | null | undefined): Omit<BuildingImage, "building_id" | "is_primary"> {
  if (!imageUrl) {
    return { image_url: "", type: "placeholder", source: null };
  }
...
function mapBuilding(old: OldBuilding): MappedBuilding {
  ...
  return {
    ...
    address: toString(old.immeuble), // immeuble is the best available proxy for address
    city: inferCity(old.zone),
    latitude: toNumber(old.latitude),
    longitude: toNumber(old.longitude),
